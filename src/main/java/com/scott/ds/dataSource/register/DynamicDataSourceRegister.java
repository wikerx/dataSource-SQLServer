package com.scott.ds.dataSource.register;

import com.scott.ds.common.encrypt.utils.AESUtil;
import com.zaxxer.hikari.HikariDataSource;
import com.scott.ds.dataSource.config.DynamicDataSourceContextHolder;
import com.scott.ds.dataSource.config.DynamicRoutingDataSource;
...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        try {
            try {
                defauleDataSourceProperties.put("url", AESUtil.decrypt(defauleDataSourceProperties.get("url").toString()));
//            logger.info("获取配置信息url：{}",defauleDataSourceProperties.get("url"));
            } catch (Exception e) {
                logger.error("master数据源url解密错误,使用原字符串读取",e);
                defauleDataSourceProperties.put("url", defauleDataSourceProperties.get("url"));
            }
            try {
                defauleDataSourceProperties.put("username", AESUtil.decrypt(defauleDataSourceProperties.get("username").toString()));
//            logger.info("获取配置信息username：{}",defauleDataSourceProperties.get("username"));
            } catch (Exception e) {
                logger.error("master数据源username解密错误,使用原字符串读取", e);
                defauleDataSourceProperties.put("username", defauleDataSourceProperties.get("username"));
            }
            try {
                defauleDataSourceProperties.put("password", AESUtil.decrypt(defauleDataSourceProperties.get("password").toString()));
//            logger.info("获取配置信息password：{}",defauleDataSourceProperties.get("password"));
            } catch (Exception e) {
                logger.error("master数据源password解密错误,使用原字符串读取", e);
                defauleDataSourceProperties.put("password", defauleDataSourceProperties.get("password"));
            }
            ...
            for (int i = 0; i < configs.size(); i++) {
                ...
                try {
                    try {
                        ...
                        logger.error(key + "数据源url解密错误,使用原字符串读取", e);
                        defauleDataSourceProperties.put("url", defauleDataSourceProperties.get("url"));
                    }
                    try {
                        defauleDataSourceProperties.put("username", AESUtil.decrypt(defauleDataSourceProperties.get("username").toString()));
                        ...
                        logger.error(key + "数据源username解密错误,使用原字符串读取", e);
                        defauleDataSourceProperties.put("username", defauleDataSourceProperties.get("username"));
                    }
                    try {
                        defauleDataSourceProperties.put("password", AESUtil.decrypt(defauleDataSourceProperties.get("password").toString()));
                        ...
                        logger.error(key + "数据源password解密错误,使用原字符串读取", e);
                        defauleDataSourceProperties.put("password", defauleDataSourceProperties.get("password"));
                    }
                    // 绑定参数
